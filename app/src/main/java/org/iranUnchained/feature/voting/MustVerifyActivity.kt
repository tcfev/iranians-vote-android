...
import org.iranUnchained.databinding.ActivityMustVerifyBinding
import org.iranUnchained.databinding.LayoutRequirementDeclineItemBinding
import org.iranUnchained.databinding.LayoutRequirementItemBinding
...
class MustVerifyActivity : BaseActivity() {
    ...
    private fun initViews() {
        ...
        val time = resolveDays(this, voteData.dueDate!!, voteData.startDate!!, getLocale())
    ...
    private fun isAllowedToSign() {
        ...
        if (SecureSharedPrefs.getIsPassportScanned(this)) {
            if (!voteData.isActive) {
                binding.mainButton.text = getString(R.string.authorize)
                declineToUser()
                return
            }
        }
        ...
        if (SecureSharedPrefs.getIsPassportScanned(this)) {
            allowToUser()
            binding.mainButton.text = voteData.metadata?.option ?: getString(R.string.authorize)
        }
    }
